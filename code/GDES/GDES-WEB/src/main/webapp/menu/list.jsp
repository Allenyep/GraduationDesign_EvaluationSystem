             ...
             <a><cite><i class="layui-icon" style="line-height:25px">&#xe68e  </i>首页</cite></a>
              <a><cite>菜单列表</cite></a>
                ...
                <a class="layui-btn layui-btn-mini" style="line-height:1.0em;margin-top:1px;float:right"
                   href="/admin/menu/index.do" title="刷新">
                    <i class="layui-icon" style="line-height:25px">&#x1002</i></a>
            </div>
        </div>
        <a class="layui-btn layui-btn-normal" role="button"
           style="margin-right: 10px;float: right" href="/admin/menu/add.do">添加菜单</a>
        <div class="layui-col-xs12 layui-col-sm12 layui-col-md12">
            <table class="layui-table"
                   lay-data="{height: 'full-110',even: true,url:'/admin/menu/list.do',limits:[10,30,50,100], limit: 10,page:true,id:'id'}"
                   lay-filter="user">
                <thead>
                <tr>
...
<script>
    layui.use(['table', 'element', 'jquery'], function () {
        ...
        table.on('tool(user)', function (obj) {
            var data = obj.data;
            if (obj.event === 'update') {
                window.location.href = '/admin/menu/update/' + data.permissionId+".do";
            }
            ...
            if (obj.event === 'delete') {
                ...
                layer.confirm('您确定要删除该菜单吗？', {
                    ...
                    layer.closeAll();
                    $.post("/admin/menu/deleteMenuById", {id: permissionId}, function (data) {
                        if (data == "success") {
                            //如果修改成功,则刷新页面
            ...
            if (obj.event === 'yes') {
                var permissionId = obj.data.permissionId;
                //已启用,设置为未启用
                $.post("/admin/menu/updateFlag", {id: permissionId, flag: 0}, function (data) {
                    if (data == "success" || data == "error1") {
                        //如果修改成功,则刷新页面
                        //window.location.reload(); //刷新当前页面
                        window.location.href = "/admin/menu/index?msg="+data+".do";
                    }
                })
            }
            if (obj.event === 'no') {
                var permissionId = obj.data.permissionId;
                //未启用,设置为已启用
                $.post("/admin/menu/updateFlag", {id: permissionId, flag: 1}, function (data) {
                    if (data == "success" || data == "error1") {
                        //如果修改成功,则刷新页面
                        //window.location.reload(); //刷新当前页面
                        window.location.href = "/admin/menu/index?msg="+data+".do";
